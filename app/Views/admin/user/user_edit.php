...
<div class="card">
    <div class="card-header bg-primary">
        ...
        <form action="<?= base_url() ?>userEditSimpan" class="formsimpan">
            <?= csrf_field(); ?>
        ...
        <div class="row">
            ...
            <div class="col-12 col-sm-6 col-md-12 col-xl-6">
                <div class="form-group">
                    ...
                    <select name="userlevel" id="userlevel" class="form-control select2 select2-danger"
                        data-dropdown-css-class="select2-danger" style="width: 100%;">
                        <option value="">Pilih Level</option>
                        <option value=""></option>
                        <?php foreach ($dataLevel as $rowLevel) : ?>
                            <option value="<?= $rowLevel['levelid'] ?>"
                                <?= ($rowLevel['levelid'] == $userlevel) ? 'selected' : '' ?>><?= $rowLevel['levelnama'] ?>
                            </option>
                        <?php endforeach ?>
                    </select>
        ...
        <div class="row">
            <div class="col-12 col-sm-6 col-md-12 col-xl-6">
                <div class="form-group">
                    ...
                    <select name="userstatus" id="userstatus" class="form-control select2 select2-danger"
                        data-dropdown-css-class="select2-danger" style="width: 100%;">
                        <option value="">Pilih Status</option>
                        <option value=""></option>
                        <option value="Aktif" <?= ($userstatus == 'Aktif') ? 'selected' : '' ?>>Aktif</option>
                        <option value="Non Aktif" <?= ($userstatus == 'Non Aktif') ? 'selected' : '' ?>>Non Aktif
                        </option>
                    </select>
                    <div class="invalid-feedback errorUserStatus"></div>
                </div>
            </div>
            <div class="col-12 col-sm-6 col-md-12 col-xl-6">
                <div class="form-group">
                    ...
                    <select name="usercabang" id="usercabang" class="form-control select2 select2-danger"
                        ...
                        <?php foreach ($dataCabang as $rowCabang) : ?>
                            <option value="<?= $rowCabang['cabang_id'] ?>"
                                <?= ($rowCabang['cabang_id'] == $usercabang) ? 'selected' : '' ?>><?= $rowCabang['cabang_nama'] ?>
                            </option>
                        <?php endforeach ?>
                    </select>
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.error) {
                        ...
                        if (err.errUserStatus) {
                            $('#userstatus').addClass('is-invalid');
                            $('.errorUserStatus').html(err.errUserStatus);
                        } else {
                            $('#userstatus').removeClass('is-invalid');
                            $('#userstatus').addClass('is-valid');
                        }
                    }
