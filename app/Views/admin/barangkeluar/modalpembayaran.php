...
<div class="modal fade" id="modalpembayaran" data-backdrop="static" data-keyboard="false" tabindex="-1"
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="<?= base_url() ?>modalPembayaranBarangKeluarSimpan" class="formsimpan" id="formTambah"
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="noinvoice" id="noinvoice" class="form-control" value="<?= $noinvoice ?>"
                            readonly>
                        <input type="hidden" name="tglinvoice" value="<?= $tglinvoice ?>">
                    ...
                    <div class="form-group">
                        <label for="">Discount Toko</label>
                        <input type="text" name="disctoko" id="disctoko" class="form-control" value="0" autocomplete="off">
                    </div>

                    <div class="form-group">
                        <label for="">Total Belanja</label>
                        <input type="text" name="totalbelanja" id="totalbelanja" class="form-control" readonly>
                    </div>

                    <div class="form-group">
                        <label for="">Jumlah Bayar</label>
                        <input type="text" name="jumlahuang" id="jumlahuang" class="form-control" autocomplete="off">
                        <div class="invalid-feedback errorJumlahUang"></div>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#disctoko').autoNumeric('init', {
            mDec: 0,
            aDec: ',',
            aSep: '.',
        })

        $('#totalbelanja').autoNumeric('init', {
            mDec: 0,
            aDec: ',',
            aSep: '.',
        })
        ...
        $('#totalbelanja').autoNumeric('set', totalbayar);




        $('#disctoko').keyup(function(e) {
            let totalbayar = $('#totalbayar').autoNumeric('get');
            let disctoko = $('#disctoko').autoNumeric('get');
            let jumlahuang = $('#jumlahuang').autoNumeric('get');

            let jumlahpembayaran = parseInt(totalbayar) - parseInt(disctoko);

            $('#totalbelanja').autoNumeric('set', jumlahpembayaran);
        ...
        $('#jumlahuang').keyup(function(e) {
            ...
            if (parseInt(jumlahuang) < parseInt(jumlahpembayaran)) {
                sisauang = 0;
            } else {
                sisauang = parseInt(jumlahuang) - parseInt(jumlahpembayaran);
            }
        ...
        $('.formsimpan').submit(function(e) {
            ...
            let disctoko = $('#disctoko').val();
            let jumlahuang = $('#jumlahuang').val();

            let jumlahpembayaran = parseInt(totalbayar) - parseInt(disctoko);

            if (jumlahuang.length == 0) {
                $('#jumlahuang').addClass('is-invalid');
                $('.errorJumlahUang').html('Tidak Boleh Kosong');
                $('#jumlahuang').focus();
            } else if (parseInt(jumlahuang) < parseInt(jumlahpembayaran)) {
                $('#jumlahuang').addClass('is-invalid');
                $('.errorJumlahUang').html('Jumlah uang kurang');
                ...
                $.ajax({
                    type: "post",
                    url: '<?= base_url() ?>modalPembayaranBarangKeluarSimpan',
                    data: formData,
                    contentType: false,
                    processData: false,
                    dataType: "json",
